...
module.exports = {
  ...
  getBuildNumberFromBuildGradle(pathToBuildGradle) {
    const content = fs.readFileSync(pathToBuildGradle, 'utf8');
    const versionCodeLine = content.match(/(\s*versionCode\s+["']?)(\d+)(["']?\s*)/g);
    const numberPattern = /\d+/g;
    const versionCode = versionCodeLine.toString().match(numberPattern);
